...
def Autoplot(path_output_run, graph):
    ...
    if graph == True:
        ...
        if 'train_metric_ia_biodiv' in df.columns :
            ...
            plt.xticks(range(0,df_final.shape[0],2))
            plt.legend(bbox_to_anchor=[1.05,1],loc=2,borderaxespad=0)
            fig.savefig(str(path) + '/metric_ia_biodiv.png', bbox_inches='tight')

        if 'train_mean_absolute_error' in df.columns :
            fig=plt.figure(figsize=[10,5], dpi=600)
            plt.plot(range(df_final.shape[0]), df_final.train_mean_absolute_error, label='train', c='steelblue')
            plt.plot(range(df_final.shape[0]), df_final.val_mean_absolute_error, label='val', c='orange')
            plt.title(path.name + ' - Mean absolute error')
            plt.ylabel("mean_absolute_error")
            plt.xlabel("epoch")
            plt.xticks(range(0,df_final.shape[0],2))
            plt.legend(bbox_to_anchor=[1.05,1],loc=2,borderaxespad=0)
            fig.savefig(str(path) + '/mean_absolute_error.png', bbox_inches='tight')

        if 'train_mean_squared_error' in df.columns :
            fig=plt.figure(figsize=[10,5], dpi=600)
            plt.plot(range(df_final.shape[0]), df_final.train_mean_squared_error, label='train', c='steelblue')
            plt.plot(range(df_final.shape[0]), df_final.val_mean_squared_error, label='val', c='orange')
            plt.title(path.name + ' - Mean squared error')
            plt.ylabel("mean_squared_error")
            plt.xlabel("epoch")
            plt.xticks(range(0,df_final.shape[0],2))
            plt.legend(bbox_to_anchor=[1.05,1],loc=2,borderaxespad=0)
            fig.savefig(str(path) + '/mean_squared_error.png', bbox_inches='tight')            

        if 'train_mean_squared_log_error' in df.columns :
            fig=plt.figure(figsize=[10,5], dpi=600)
            plt.plot(range(df_final.shape[0]), df_final.train_mean_squared_log_error, label='train', c='steelblue')
            plt.plot(range(df_final.shape[0]), df_final.val_mean_squared_log_error, label='val', c='orange')
            plt.title(path.name + ' - Mean squared log error')
            plt.ylabel("mean_squared_log_error")
            plt.xlabel("epoch")
            plt.xticks(range(0,df_final.shape[0],2))
            plt.legend(bbox_to_anchor=[1.05,1],loc=2,borderaxespad=0)
            fig.savefig(str(path) + '/mean_squared_log_error.png', bbox_inches='tight')    


        if 'train_r2score_mean_by_site_of_log' in df.columns :
            fig=plt.figure(figsize=[10,5], dpi=600)
            plt.plot(range(df_final.shape[0]), df_final.train_r2score_mean_by_site_of_log, label='train', c='steelblue')
            plt.plot(range(df_final.shape[0]), df_final.val_r2score_mean_by_site_of_log, label='val', c='orange')
            plt.title(path.name + ' - Mean R2 score by sites of log(n+1)')
            plt.ylabel("r2score_mean_by_site_of_log")
            plt.xlabel("epoch")
            plt.xticks(range(0,df_final.shape[0],2))
            plt.legend(bbox_to_anchor=[1.05,1],loc=2,borderaxespad=0)
            fig.savefig(str(path) + '/r2score_mean_by_site_of_log.png', bbox_inches='tight')    

        if 'train_r2score_mean_by_site' in df.columns :
            fig=plt.figure(figsize=[10,5], dpi=600)
            plt.plot(range(df_final.shape[0]), df_final.train_r2score_mean_by_site, label='train', c='steelblue')
            plt.plot(range(df_final.shape[0]), df_final.val_r2score_mean_by_site, label='val', c='orange')
            plt.title(path.name + ' - Mean R2 score by sites')
            plt.ylabel("r2score_mean_by_site")
            plt.xlabel("epoch")
            plt.xticks(range(0,df_final.shape[0],2))
            plt.legend(bbox_to_anchor=[1.05,1],loc=2,borderaxespad=0)
            fig.savefig(str(path) + '/r2score_mean_by_site.png', bbox_inches='tight')    